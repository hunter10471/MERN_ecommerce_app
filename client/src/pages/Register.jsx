...
export const Register = () => {
  ...
  return (
    ...
    <form onSubmit={handleSubmit} className='flex shadow-xl bg-white flex-col ring-primaryLight ring-2 rounded-sm p-5 sm:p-10 mx-auto' >
        ...
        <label className='text-sm md:text-base font-medium text-stone-600'>Username</label>
        <input required onChange={(e)=>setUsername(e.target.value)} className='placeholder:italic hover:bg-slate-100 transition-all duration-200 border-2 hover:border-blue-400 my-2 sm:my-4 text-sm md:text-base  px-2 sm:px-4 min-w-[200px] py-2 focus:outline-none focus:border-blue-400 ring-2 ring-transparent' type="text" placeholder='Username' />
        <label className='text-sm md:text-base font-medium text-stone-600'>Email</label>
        <input type='email'  required onChange={(e)=>setEmail(e.target.value)} className='placeholder:italic hover:bg-slate-100 transition-all duration-200 border-2 hover:border-blue-400 my-2 sm:my-4 text-sm md:text-base  px-2 sm:px-4 min-w-[200px] py-2 focus:outline-none focus:border-blue-400 ring-2 ring-transparent' placeholder='Email' />
        <div className='relative'>
        <label className='text-sm md:text-base font-medium text-stone-600'>Password</label>
        ...
        <input minLength={8} required onChange={(e)=>setPassword(e.target.value)} className='placeholder:italic hover:bg-slate-100 transition-all duration-200 w-full border-2 hover:border-blue-400 my-2 sm:my-4 text-sm md:text-base  px-2 sm:px-4 min-w-[200px] py-2 focus:outline-none focus:border-blue-400 ring-2 ring-transparent' type={visiblePassword === true ? 'text' : 'password'} placeholder='Password' />
        </div>
        <div className='flex items-center text-[10px] mb-4 md:text-[14px] text-stone-600'> <LockIcon fontSize='' /> <span className='ml-1 sm:mt-[3px]' >Password must be longer than 8 characters. </span> </div>
        <div className='relative'>
        <label className='text-sm md:text-base font-medium text-stone-600'>Confirm Password</label>
        <span onClick={()=>setVisiblePassword2(!visiblePassword2)}  className='absolute right-4 bottom-[0.9rem] sm:bottom-[1.5rem] cursor-pointer text-gray-500 '>{visiblePassword2===true ? <VisibilityOffIcon/> : <VisibilityIcon/>}</span>
        <input required onChange={(e)=>setConfirmPassword(e.target.value)} className='placeholder:italic hover:bg-slate-100 transition-all duration-200 w-full border-2 hover:border-blue-400 my-2 sm:my-4 text-sm md:text-base  px-2 sm:px-4 min-w-[200px] py-2 focus:outline-none focus:border-blue-400 ring-2 ring-transparent' type={visiblePassword2 === true ? 'text' : 'password'} placeholder='Confirm Password' />
        </div>
        ...
        <span className='text-sm md:text-base ml-2 '>Click this to recieve our monthly newsletter and our offers.</span>
        </div>
        <span className='text-sm md:text-base ml-2 my-2'>By clicking register you agree to our <u className='cursor-pointer text-blue-800 font-medium'>terms</u>  and <u className='cursor-pointer text-blue-800 font-medium'>conditions</u>.</span>
        <button className=' px-2 sm:px-4 py-2 mt-4 bg-primary hover:bg-primaryLight text-sm md:text-base '>Register</button>
        <div className='w-full flex justify-center mt-4'><div id="my-signin2" data-onsuccess="onSignIn"></div></div>
        <span className='text-sm md:text-base text-center mt-4'>Already have an account? <b className='underline ml-1 cursor-pointer'> <Link to='/login'> Login </Link></b></span>
    </form>
      </div>
      </motion.div>
  )
