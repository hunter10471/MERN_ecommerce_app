  ...
  return (
    <motion.div initial={{opacity:0}} animate={{opacity:1,transition:{duration:0.25,ease:'easeIn'}}} exit={{opacity:0,transition:{duration:0.12,ease:'easeIn'}}} className='px-2 pr-4 my-10'>
        ...
        <div className='flex flex-col items-center sm:mx-4'>
            <CartList/>
        <button onClick={()=>handleReset()} className='my-10 px-4 py-1 font-medium w-fit  text-xs md:text-sm lg:text-base rounded border-2 border-primary text-primary hover:text-primaryLight hover:border-primaryLight'>Empty Cart</button>
        </div>
