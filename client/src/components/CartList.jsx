    ...
    <table className='w-full lg:w-[700px] xl:w-[1000px]'>
      ...
      {cart.products.length !== 0 ? (
        cart.products.map((el, index) => {
          return (
            <tr key={index}>
              <td className='flex sm:p-4 lg:p-6 justify-center sm:flex-row flex-col items-center'>
                <img
                  className='rounded-[50%] object-cover w-[60px] h-[60px] lg:w-[80px] lg:h-[80px]'
                  src={el.productImg}
                  alt='product'
                />
                <div className='ml-4 flex flex-col font-medium text-sm md:text-base lg:text-lg cursor-pointer hover:text-gray-700'>
                  <span onClick={() => navigate('/product?id=' + el._id)}>
                    {' '}
                    {el.productName}{' '}
                  </span>
                  <span className='text-slate-400 text-[10px] sm:text-xs md:text-sm mb-4'>
                    SKU : {el._id}
                  </span>
                </div>
              </td>
        ...
        <td
          colSpan={4}
          className='text-center font-bold italic text-xl py-20 text-gray-300 uppercase '
        >
          Add products to see here{' '}
          <AddShoppingCartIcon
            className='mx-2 rotate-[-12deg]'
            fontSize='large'
          />
        </td>
      )}
